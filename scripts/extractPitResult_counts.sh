...
function extractMutation() {
	...
	if [ -n "$success" ]; then
		percentKilled=$(grep -oP 'Generated \d+ mutations Killed \K\d+' $dir/$fileName)
...
mkdir $HOME_DIR/result/mutation_counts

cat $HOME_DIR/repositories.txt |
while IFS=" " read -r subN noUse noUse2; do
    mutFile=$HOME_DIR/result/mutation_counts/$subN.csv
